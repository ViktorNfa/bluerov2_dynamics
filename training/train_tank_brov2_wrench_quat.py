"""
End-to-end example (wrench dataset, quaternion state):
...
from fossen.BlueROV2_wrench import BlueROV2, quat_normalize, quat_to_rotation_matrix, quat_derivative, quat_to_euler, quat_to_yaw
...
def animate_xy_four(
    ...
    All trajectories must be the same shape (T, >=7).

    Quaternion state expected:
      [x, y, z, qw, qx, qy, qz, ...]
    Yaw for arrow is extracted from quaternion (for visualization only).
    """
    assert true_traj.shape == koop_traj.shape == fossen_traj.shape == di_traj.shape
    T, n = true_traj.shape
    assert n >= 7, "Expected at least 7D state with quaternion at indices 3:7."
...
def load_dataset(csv_path: Path):
    ...
    # State and input columns for wrench dataset (quaternion orientation)
    STATE_COLS = ["x","y","z","qw","qx","qy","qz","u","v","w","p","q","r"]
    INPUT_COLS = ["Fx","Fy","Fz","Mx","My","Mz"]

    # If a legacy Euler dataset is loaded by mistake, convert it (minimal convenience).
    if all(c in df.columns for c in ["phi", "theta", "psi"]) and not all(c in df.columns for c in ["qw","qx","qy","qz"]):
        print("[warn] Euler angles detected in dataset; converting to quaternions...")

        # Build quaternion columns from Euler angles (Z-Y-X)
        phi = df["phi"].to_numpy(dtype=float)
        theta = df["theta"].to_numpy(dtype=float)
        psi = df["psi"].to_numpy(dtype=float)

        # vectorized Euler->quat
        c1 = np.cos(phi * 0.5); s1 = np.sin(phi * 0.5)
        c2 = np.cos(theta * 0.5); s2 = np.sin(theta * 0.5)
        c3 = np.cos(psi * 0.5); s3 = np.sin(psi * 0.5)
        ...
        df["qw"] = q[:, 0]; df["qx"] = q[:, 1]; df["qy"] = q[:, 2]; df["qz"] = q[:, 3]
    ...
    # Normalize quaternions in the dataset (cheap safety)
...
def simulate_physics(x0: np.ndarray, U_seq: np.ndarray, dt: float, rov: BlueROV2):
    ...
    Returns trajectory of shape (len(U_seq)+1, 13).
    """
    H = len(U_seq)
    ...
    for k in range(H):
        ...
        # Keep quaternion unit-length (Euler integration drift)
...
def estimate_di_gains(X_train: np.ndarray,
    ...
    """
    V = X_train[:, 7:10]    # body linear velocities (u,v,w)
    W = X_train[:, 10:13]   # body angular rates (p,q,r)
...
def simulate_double_integrator(x0: np.ndarray,
    ...
    Discrete DI in body frame, driven by wrench (6D):
      ...
      x_{k+1} = x_k + dt * (R_b2n(q) @ v_k)
      q_{k+1} = q_k + dt * q_dot(q_k, w_k)   (then normalize)
    """
    H = len(U_seq)
    traj = np.zeros((H + 1, x0.shape[0]))
    traj[0] = x = x0.copy()
    ...
    for k in range(H):
        ...
        # Integrate pose
        Rb2n = quat_to_rotation_matrix(q)
        pos_next = pos + dt * (Rb2n @ v)
        q_next = q + dt * quat_derivative(q, w)
...
def main():
    ...
    print("[ok] Koopman model fitted (wrench input, quat orientation).")
    ...
    # Koopman rollout
    predK = model.simulate(x0, U_seq)                       # (horizon+1, 13)
    # Keep quaternion unit-length (in case model drifts)
    for k in range(len(predK)):
        predK[k, 3:7] = quat_normalize(predK[k, 3:7])

    # Fossen/BlueROV2 rollout
    rov = BlueROV2()
    predF = simulate_physics(x0, U_seq, dt, rov)            # (horizon+1, 13)
    ...
    animate_xy_four(
        ...
        di_traj=predD,
        dt=dt,
        save_path="media/csv_true_vs_models_wrench_quat.gif",
    ...
    for k in range(min(200, horizon)):
        ...
        Kphi, Ktheta, Kpsi = quat_to_euler(predK[k, 3:7])
        Tphi, Ttheta, Tpsi = quat_to_euler(true_traj[k, 3:7])

        Kang = np.rad2deg([Kphi, Ktheta, Kpsi])
        Tang = np.rad2deg([Tphi, Ttheta, Tpsi])
