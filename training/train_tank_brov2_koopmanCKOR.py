...
End-to-end example (recorded data):
  ...
  2) Fit a cKOR Koopman model (full or Ny-cKOR sketch)
  3) Report one-step and multi-step RMSE (H=1,10,100)
...
from Koopman.koopmanCKOR import KoopmanCKOR
...
# CKOR hyper-parameters (tuned on tank dataset split)
STATE_GAMMA = 0.1
CONTROL_KERNEL = "linear"    # {"linear", "rbf"}
CONTROL_GAMMA = 1.0
RIDGE = 1e-3
...
def animate_xy_true_vs_koopman(
    ...
    Shows/saves a side-by-side x-y animation with a heading indicator from psi (yaw).
    ...
    for ax, name in [(axL, "TRUE (BlueROV2)"), (axR, "CKOR PREDICTION")]:
...
def multistep_rmse_endpoint_koopman(model: KoopmanCKOR,
                                    X_test: np.ndarray,
                                    U_test: np.ndarray,
                                    H: int) -> float:
    """
    Strict H-step-ahead RMSE (endpoint only), identical to KoopmanCKOR.multistep_rmse.
    """
...
def main():
    ...
    mode_name = "Ny-cKOR" if model.mode_ == "nystrom" else "cKOR"
    print(f"[ok] {mode_name} model fitted. lift_dim={model.lift_dim_}")

    # ------------------------------------------------------------------
    #  3a)  Metrics
    # ------------------------------------------------------------------
    rmse_1_kgen = multistep_rmse_endpoint_koopman(model, X_test, U_test, H=1)
    ...
    print("\n[metrics] CKOR endpoint RMSE:")
    print("  Model          | 1-step RMSE | 10-step RMSE | 100-step RMSE")
    print("  -------------- | ----------- | ------------ | -------------")
    print(f"  {mode_name:14s} | {rmse_1_kgen:11.6f} | {rmse_10_kgen:12.6f} | {rmse_100_kgen:13.6f}")

    # ------------------------------------------------------------------
    #  4)  Short open-loop demo (CKOR vs True)
    # ------------------------------------------------------------------
    start = 0
    ...
    animate_xy_true_vs_koopman(
        ...
        koop_traj=predK,
        dt=dt,
        save_path="media/csv_true_vs_ckor.gif",
        title=f"Recorded CSV: True vs. {mode_name} (top view)",
        tail_secs=10.0,
        speed=1.0,
        dpi=130,
    )
