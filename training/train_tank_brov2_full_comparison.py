...
def plot_2d_trajectories_with_depth(
    ...
    if pinc_traj is not None:
        traj_specs.append((pinc_traj, "PINc", "-."))

    # Horizon based on available trajectories
    max_steps = int(seconds / max(dt, 1e-9))
    horizon = min(
        max_steps,
        *[traj.shape[0] for traj, _, _ in traj_specs],
    )
    ...
    for X, lbl in zip(trajs, labels):
        text_lines.append(f"z_{lbl} = {X[-1, 2]:.2f} m")
    txt = "\n".join(text_lines)
