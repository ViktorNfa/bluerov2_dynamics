...
class BlueROV2:
    ...
    def _thruster_force_from_input(self, V, i, dt):
        ...
        # 2) dynamic lag
        F = (-140.3*V9 + 389.9*V7 - 404.1*V5 + 176.0*V3 + 8.9*V)
        F_dyn = self.thruster_lags[i].step(F, dt) 
        
        # Comment this line to deactivate the thruster lag dynamics
        F = F_dyn

        return float(F) # 1-element array -> scalar
...
class Tether:
    ...
    Example usage:
      ...
      # then store x_teth_init in main code, e.g. rov.tether_state = x_teth_init
    """
    ...
    def __init__(self,
                 ...
                 E_modulus=6.437e7, # I think this value might be too high in the paper
                 drag_normal=1.2,
                 drag_tangent=0.01,
                 c_internal=100.0,
    ...
    def dynamics(self, x_teth, anchor_pos, rov_pos, rov_vel, current_ned):
        ...
        for k in range(self.n):
            ...
            if L_k > self.l0: # slack → no tension
                T_k = (self.Et * self.Across / self.l0) * (L_k - self.l0) * r_hat
            else:
                T_k = np.zeros(3)
            T.append(T_k)
